...
class OpenAIChatCompletionRequest(BaseModel):
    ...
    user: str = Field(..., description="The ref of the user")
    ...
    role: str = Field("assistant", description="The role that the chatbot should play")
    template: str = Field("base", description="The template to use for the response")


class OpenAIClient(Client):
    """
    A class used to represent a client for the OpenAI API.
    """
    ...
    async def post_embeddings(self, request: OpenAIEmbeddingRequest):
        response = await self.post("/embeddings", json=request.dict())
        assert isinstance(response, dict)
        return OpenAIEmbeddingResponse(**response)

    async def chat_completion(self, request: OpenAIChatCompletionRequest, content: str):
        response = await self.post(
            "/chat/completions",
            json=OpenAIChatGptRequest().chain(content, request.prompt).dict(),
        )
        assert isinstance(response, dict)
